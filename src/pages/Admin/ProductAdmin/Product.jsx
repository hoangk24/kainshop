...
import {CloseCircleOutlined, EditOutlined, MinusCircleOutlined, PlusOutlined, UploadOutlined} from "@ant-design/icons";
...
function Product(props) {
            ...
            render:(poster)=>{
                return <div  >
                    <Avatar.Group
                        ...
                        size="large"
                    >
                        {poster?.map((item,index)=><Avatar key={index} src={item.url}/>)}
                    </Avatar.Group>
                </div>
            }
        },
        ...
        },
        {
            ...
            render:size=>{
                return <div style={{whiteSpace:'nowrap'}}>
                    {size.map((item,index)=><Tag color={'green'}>{item}</Tag>)}
                </div>
            }
        },
        {
            ...
            render:color=>{
                return <div style={{whiteSpace:'nowrap'}}>
                    {color.map((item,index)=><Tag color={'blue'}>{item}</Tag>)}
                </div>
            }
        },
        {
        ...
        },
        {
            ...
            render:(text,record)=>{
                return <Space>
                   ...
                   >
                       <Button
                           icon={<CloseCircleOutlined/>}>Xóa</Button>
                   </Popconfirm>
                </Space>
            }
        }
    ...
    const handleSubmit = useCallback(async (values) => {
        ...
        await adminApi.addProduct(formData).then(res => {
    ...
    const deleteProduct = async(id)=>{
        ...
        await adminApi.deleteProduct(id).then(res=>{
    ...
    return (
        <div className="product-admin">
            ...
            <Space>
                ...
                >Thêm Danh mục và nhà sản xuất</Button>
            </Space>
            <Modal style={{position: 'relative'}} visible={addModel} onCancel={closeAddModel} onOk={form.submit}>
                ...
                >
                    <Form.Item label={'Tển sản phẩm'} name={'name'}
                                ...
                                {fields.map((field, index) => (
                                    ...
                                    >
                                        <Form.Item
                                            ...
                                            noStyle
                                        >
                                            <Input placeholder="Hãy nhập size" style={{width: '60%'}}/>
                                        </Form.Item>
                                        {fields.length > 1 ? (
                                            <MinusCircleOutlined
                                ...
                                {fields.map((field, index) => (
                                    ...
                                    >
                                        <Form.Item
                                            ...
                                            noStyle
                                        >
                                            <Input placeholder="Hãy nhập màu" style={{width: '60%'}}/>
                                        </Form.Item>
                                        {fields.length > 1 ? (
                                            <MinusCircleOutlined
                                                className="dynamic-delete-button"
                    ...
                    rules={[{required:true,message:'Phải chọn 1 trong các danh mục'}]}
                    >
                    ...
                    <Form.Item label={'Nhà sản xuất'} name={'NSX'}
                               rules={[{required:true,message:'Phải chọn 1 nhà sản xuất sau khi chọn danh mục'}]}
                    >
                        <Select defaultValue={'Chưa chọn'} >
                            {nsx?.map((item,index)=><Option value={item} key={index}>{item}</Option>)}
                        </Select>
                    </Form.Item>
                </Form>
            </Modal>
            <Table
                ...
                columns={column} loading={loadingProduct}
                dataSource={productList}
                pagination={false}
            />
            <div style={{display:'flex',justifyContent:'flex-end',marginTop:'1rem'}}>
                <Pagination defaultCurrent={1} total={totalDocuments} current={page}
