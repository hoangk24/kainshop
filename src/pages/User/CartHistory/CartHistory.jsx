...
import { Steps, Divider, Table, Space, Button, message, Tabs } from "antd";
...
export default function CartHistory() {
  ...
  const renderCart = (cartList) =>
    cartList.map((item, index) => {
      ...
      return (
        <div className='' style={{ marginBottom: "2rem" }} key={index}>
          <div
            style={{
              textAlign: "center",
              marginBottom: "10px",
          ...
          >
            Ngày đặt hàng:{moment().format(item.createdAt)}
          </div>
          <Steps
          ...
          <Space style={{ marginTop: "1rem" }}>
            ...
            <span>
              Số điện thoại nhận hàng: <b>(+84) {item.phoneNumber}</b>
            </span>
          </Space>
          <Table
            dataSource={list}
        ...
        <Tabs style={{ padding: "2rem" }}>
          <TabPane tab='Đang xử lý' key={1}>
            <div className='cart-item'>
              {renderCart(cart.filter((item) => item.state !== -1))}
            </div>
          </TabPane>
          <TabPane tab='Hủy' key={2}>
            <div className='cart-item'>
              {renderCart(cart.filter((item) => item.state === -1))}
            </div>
          </TabPane>
